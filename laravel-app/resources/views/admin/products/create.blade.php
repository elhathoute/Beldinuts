...
<div class="min-h-screen bg-gray-50 py-8 mt-16">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form method="POST" action="{{ route('admin.products.store', app()->getLocale()) }}">
                @csrf
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="number" step="0.01" name="retail_price" id="retail_price" 
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-light-gold focus:border-light-gold" 
                                       value="{{ old('retail_price') }}" placeholder="Ex: 2.5">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="wholesale_price" id="wholesale_price" 
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-light-gold focus:border-light-gold" 
                                       value="{{ old('wholesale_price') }}" placeholder="Ex: 2.0">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="unit" id="unit" required 
                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-light-gold focus:border-light-gold">
                                    <option value="piece" {{ old('unit', 'piece') == 'piece' ? 'selected' : '' }}>{{ __('admin.unit_piece') }}</option>
                                    <option value="gram" {{ old('unit') == 'gram' ? 'selected' : '' }}>{{ __('admin.unit_gram') }}</option>
                                </select>
